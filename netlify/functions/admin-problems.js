...
exports.handler = async function (event) {
  try {
    ...
    if (SUPABASE_URL && SUPABASE_SERVICE_ROLE_KEY) {
      try {
        ...
        if (event.httpMethod === 'POST') {
          ...
          const { data, error } = await sb.from('problems').insert([body]).select().single()
          if (error) throw error
        ...
        if (event.httpMethod === 'PUT') {
          ...
          const { data, error } = await sb.from('problems').update(body).eq('id', body.id).select().single()
          if (error) throw error
          return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data }) }
        }
        ...
        if (event.httpMethod === 'DELETE') {
          ...
          const { data, error } = await sb.from('problems').delete().eq('id', id).select().single()
          if (error) throw error
          return { statusCode: 200, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data }) }
        }
      } catch (err) {
    ...
    if (event.httpMethod === 'DELETE') {
      // expect id as query param or in body
      const qs = (event.queryStringParameters && event.queryStringParameters.id) || null
      let id = qs
      if (!id) {
        const b = JSON.parse(event.body || '{}')
